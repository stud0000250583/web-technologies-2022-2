export const TicTacToe = {
        ...
        this.boxes.forEach(box => {
            box.addEventListener('click', event => {
                ...
                if (!this.checkHasEmptyBlocks()) {
                    ...
                    // проверяем, является ли ход победным при оставшихся пустых клетках
                    if (this.checkForWin())
                        this.setGameEndStatus()
                }
    ...
    setBlockValue(target, clear) {
        let pos = this.getBlockPosition(target).map(x => x - 1)
        if (clear)
            this.matrix[pos[0]][pos[1]] = null
        else
            this.matrix[pos[0]][pos[1]] = this.getCurrentTurnValue()
    },
