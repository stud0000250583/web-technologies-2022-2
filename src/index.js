...
const snake = {
     ...
     */
    makeStep() {
        let destination = this.getNextStepHeadPoint()
        let snake = this.getBody()

        for (let i = 0; i < snake.length; i++) {
            let start = { x: snake[i].x, y: snake[i].y }
            snake[i].x = destination.x
            snake[i].y = destination.y
...
const food = {
     ...
     */
    isOnPoint(point) { 
        let foodCoordinates = this.getCoordinates()
        return point.x == foodCoordinates.x && point.y == foodCoordinates.y
    },
};
...
const game = {
    ...
    getRandomFreeCoordinates() {
        ...
        let randomFreeCoordinates = { x, y }
        for (let i = 0; i < this.map.usedCells.length; i++)
            if (this.map.usedCells[i].x == randomFreeCoordinates.x && this.map.usedCells[i].y == randomFreeCoordinates.y)
    ...
    canSetDirection(direction) {
        ...
        if (lastStepDirection == 'up')
            return direction != 'down'
        else if (lastStepDirection == 'down')
            return direction != 'up'
        else if (lastStepDirection == 'left')
    ...
    canMakeStep() {
        // return true
        if (this.snake.isOnPoint(this.snake.getNextStepHeadPoint()))
            return false
        return true
    },
};
